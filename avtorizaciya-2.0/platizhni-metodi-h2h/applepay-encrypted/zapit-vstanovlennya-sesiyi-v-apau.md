---
description: >-
  Запит за допомогою якого встановлюється сесія в aPау. У відповідь повертаються
  токенізовані картки клієнта
---

# Запит встановлення сесії в aPау

\{{url\}}/ecom/jws/payments/apple\_pay/get\_session\_v1

### Вхідні параметри частини payload JWS :

| Параметр          | Опис                                                                                                                                                                                  | Формат даних | Обов'язковіть | Приклад                                                                      |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ------------- | ---------------------------------------------------------------------------- |
| validationUrl     | URL-адреса, на яку Apple надсилає запит для валідації сесії платежу                                                                                                                   | string(1000) | Так           | [apple-pay-gateway.apple.com](http://apple-pay-gateway.apple.com/)           |
| merchantId        | apple merchant identifier. параметр отримуєте від банка                                                                                                                               | string(255)  | Так           | YN\_O1aEEFqF6Jal4B5qHy7b7                                                    |
| domain            | доменне ім'я (URL), яке використовується для авторизації                                                                                                                              | string(255)  | Так           | [checkout.develop.bankalliance.ua](http://checkout.develop.bankalliance.ua/) |
| merchantRequestId | <p>унікальний ідентифікатор згенерований системою мерчанта, використовується для можливості дізнатися статус операції якщо запит закінчився невідомою помилкою чи дісконектом<br></p> | string(50)   | Так           | merchantRequestId\_cfc297a1e1e91                                             |

### Вихідні параметри частини payload JWS :

| Параметр                  | Опис                                                                                                                                                                                 | Формат даних | Приклад                                     |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------ | ------------------------------------------- |
| applePaySessionData       |                                                                                                                                                                                      | object       |                                             |
| epochTimestamp            | час початку дії сесії (у мілісекундах)                                                                                                                                               | string       | 2024-09-30T15:06:05.274+03:00               |
| expiresAt                 | час закінчення дії сесії (у мілісекундах)                                                                                                                                            | string       | 2024-09-30T16:06:05.274+03:00               |
| merchantSessionIdentifier | ідентифікатор сесії мерчанта                                                                                                                                                         | string       | SSHB8FD6B91EB2B4C909990AE16F652A8BB\_BB8E.. |
| nonce                     | унікальний токен, який використовується для безпеки                                                                                                                                  | string       | 0162edba                                    |
| merchantIdentifier        | ідентифікатор зареєстрованого мерчанта                                                                                                                                               | string       | C20AFFF8FC4092AB90F781FFA08E5C49..          |
| domainName                | доменне ім'я, де була ініційована сесія                                                                                                                                              | string       | checkout.develop.bankalliance.ua            |
| displayName               | ім'я мерчанта, яке буде відображене                                                                                                                                                  | string       | merchant.aliiancebank.ua                    |
| signature                 | підпис, який використовується для підтвердження дійсності сесії                                                                                                                      | string       | 308006092a864886f70d010702a080308002010..   |
| merchantRequestId         | <p>унікальний ідентифікатор згенерований системою мерчанта, використовується для можливості дізнатися статусоперації якщо запит закінчився невідомою помилкою чи дісконектом<br></p> | string       | merchantRequestId\_cfc297a1e1e91            |
| applePaySessionId         | ідентифікатор сесії Apple Pay                                                                                                                                                        | string       | f40b099c-38f3-46f9-a989-52818018f557        |



### Приклади:&#x20;

<details>

<summary>JWS Payload — тіло запиту перед підписанням</summary>

```json
{
    "merchantId": "YN_O1aEEFqF6Jal475qHy7b7",
    "validationUrl": "https://apple-pay-gateway.apple.com/paymentservices/startSession",
    "merchantRequestId": "merchantRequestId_cfc297a1e1e91",
    "domain": "checkout.develop.bankalliance.ua"
}
```

</details>



<details>

<summary>JWS Payload — тіло відповіді перед підписанням</summary>

```json
{
    "applePaySessionData": {
        "epochTimestamp": "2024-09-30T15:06:05.274+03:00",
        "expiresAt": "2024-09-30T16:06:05.274+03:00",
        "merchantSessionIdentifier": "SSHB8FD6B91EB2B4C52A8BB_BB8E62003687F8FCC159B2B83AAFC02DB625F1F1E3997CCC2FE2CFD11F636558",
        "nonce": "016edba",
        "merchantIdentifier": "C20AFFF8FC4092AB90F781FFA08E5C49434674F64EB2A187A7FE3094EB1C8EF5",
        "domainName": "checkout.develop.bankalliance.ua",
        "displayName": "merchant.aliiancebank.ua",
        "signature": ""
    },
    "merchantRequestId": "merchantRequestId_cfc297a1e1e91",
    "applePaySessionId": "f45b099c-38f3-46f9-a989-52818018f557"
}
```

</details>
